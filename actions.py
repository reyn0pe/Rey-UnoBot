...
def do_skip(bot, player, job_queue=None):
    ...
    if skipped_player.waiting_time > 0:
        ...
        send_async(bot, chat.id,
                   text=__("Menunggu waktu untuk melewati pemain ini memiliki "
                        "dikurangi menjadi {time} detik.\n"
                        "Pemain berikutnya: {name}", multi=game.translate)
                   .format(time=n,
                           name=display_name(next_player.user))
        )
        logger.info("{player} dilewati! "
        ...
        try:
            ...
            send_async(bot, chat.id,
                       text=__("{name1} kehabisan waktu "
                            "dan telah dihapus dari game!\n"
                            "Pemain berikutnya: {name2}", multi=game.translate)
            ...
            send_async(bot, chat.id,
                       text=__("{name} kehabisan waktu "
                               "dan telah dihapus dari game!\n"
                               "Permainan berakhir.", multi=game.translate)
...
def do_play_card(bot, player, result_id):
    ...
    if game.choosing_color:
        send_async(bot, chat.id, text=__("Silahkan pilih warna", multi=game.translate))
    ...
    if len(player.cards) == 0:
        send_async(bot, chat.id,
                   text=__("{name} Winner! Please Join 🔥 @ReyUpdatesCH For Support", multi=game.translate)
...
def do_draw(bot, player):
    ...
    try:
        ...
        send_async(bot, player.game.chat.id,
                   text=__("Tidak ada lagi kartu di geladak.",
...
def do_call_bluff(bot, player):
    ...
    if player.prev.bluffing:
        send_async(bot, chat.id,
                   text=__("Bluff dipanggil! Memberikan 4 kartu kepada {name}",
                           multi=game.translate)
        ...
        game.draw_counter += 2
        send_async(bot, chat.id,
                   text=__("{name1} tidak menggertak! Memberikan 6 kartu kepada {name2}",
                           multi=game.translate)
...
def start_player_countdown(bot, game, job_queue):
    ...
    if game.mode == 'fast':
        ...
        logger.info("Hitung mundur dimulai untuk pemain: {player}. {time} detik."
