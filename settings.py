...
def show_settings(update: Update, context: CallbackContext):
    ...
    if update.message.chat.type != 'private':
        send_async(context.bot, chat.id,
                   text=_("Harap edit pengaturan Anda dalam obrolan pribadi dengan "
                          "bot."))
        return
...
def kb_select(update: Update, context: CallbackContext):
    ...
    if option == '📊':
        ...
        us.stats = True
        send_async(context.bot, chat.id, text=_("Statistik yang diaktifkan!"))

    elif option == '🌍':
        kb = [[locale + ' - ' + descr]
              for locale, descr
              in sorted(available_locales.items())]
        send_async(context.bot, chat.id, text=_("Pilih lokal"),
                   reply_markup=ReplyKeyboardMarkup(keyboard=kb,
                                                    one_time_keyboard=True))
        ...
        us.games_played = 0
        us.cards_played = 0
        send_async(context.bot, chat.id, text=_("Statistik yang dihapus dan dinonaktifkan!"))
...
def locale_select(update: Update, context: CallbackContext):
    ...
    if option in available_locales:
        ...
        us.lang = option
        _.push(option)
        send_async(context.bot, chat.id, text=_("Tetapkan lokal!"))
        _.pop()
