...
class Player(object):
    ...
    def playable_cards(self):
        ...
        self.logger.debug("Kartu terakhir adalah " + str(last))
    ...
    def _card_playable(self, card):
        ...
        is_playable = True
        last = self.game.last_card
        self.logger.debug("Memeriksa kartu " + str(card))

        if (card.color != last.color and card.value != last.value and
                not card.special):
            self.logger.debug("Warna atau nilai kartu tidak cocok")
            is_playable = False
        elif last.value == c.DRAW_TWO and not \
                card.value == c.DRAW_TWO and self.game.draw_counter:
            self.logger.debug("Pemain harus menggambar dan tidak bisa membalas")
            is_playable = False
            ...
            self.logger.debug("Tidak dapat memainkan pemilih warna pada yang lain")
            is_playable = False
        elif not last.color:
            self.logger.debug("Kartu terakhir tidak memiliki warna")
            is_playable = False
