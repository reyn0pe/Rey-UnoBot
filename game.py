...
class Game(object):
    ...
    def turn(self):
        ...
        self.logger.debug("Pemain Pemain Selanjutnya")
    ...
    def play_card(self, card):
        ...
        self.logger.info("Bermain kartu " + repr(card))
        if card.value == c.SKIP:
            self.turn()
        elif card.special == c.DRAW_FOUR:
            self.draw_counter += 4
            self.logger.debug("Draw counter bertambah 4")
        elif card.value == c.DRAW_TWO:
            self.draw_counter += 2
            self.logger.debug("Draw counter bertambah 2")
